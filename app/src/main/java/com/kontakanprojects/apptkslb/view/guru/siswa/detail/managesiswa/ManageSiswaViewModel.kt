...
class ManageSiswaViewModel : ViewModel() {

    private var _siswa: MutableLiveData<ResponseDetailSiswa?>? = null
    private var _kelas: MutableLiveData<ResponseKelas?>? = null

    private val TAG = ManageSiswaViewModel::class.simpleName

    fun mapel(): LiveData<ResponseKelas?> {
        ...
        client.enqueue(object : Callback<ResponseKelas> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    val responseMapel =
                        ResponseKelas(message = message, status = status)
                    _kelas?.postValue(responseMapel)
