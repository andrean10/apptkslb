...
import com.kontakanprojects.apptkslb.databinding.FragmentProfileGuruBinding
...
class ProfileGuruFragment : Fragment(), View.OnClickListener {
    ...
    private lateinit var binding: FragmentProfileGuruBinding
    private val viewModel by viewModels<ProfileGuruViewModel>()

    private lateinit var resulDetailGuru: ResultDetailGuru
    ...
    private lateinit var spinnerMapel: SmartMaterialSpinner<*>

    private lateinit var tvUiUpdated: TextView
    private lateinit var tvUiUpdated2: TextView

    private var imageFile: File? = null
    private var imageUri: Uri? = null
    private var currentImagePath: String? = null

    private var idGuru = 0
    private var idRole = 0

    private var idKelasFromSpinner = 0
    private var idMapelFromSpinner = 0

    private var selectedFromAPI = false
    ...
    companion object {
        ...
        private const val MAPEL_MUST_BE_FILLED = "Pilihan Mata Pelajaran Harus Di Isi"
        private const val OLD_PASSWORD_MUST_BE_FILLED = "Password Lama Harus Di Isi"
        private const val NEW_PASSWORD_MUST_BE_FILLED = "Password Baru Harus Di Isi"
        private const val NEWAGAIN_PASSWORD_MUST_BE_FILLED = "Password Baru Harus Di Isi Ulang"
        ...
        private const val MIN_FIVE_CHARACTER_PASSWORD = "Minimal 5 karakter password"
    }
    ...
    override fun onCreateView(
        ...
        binding = FragmentProfileGuruBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            fabUploadImage.setOnClickListener(this@ProfileGuruFragment)
            layoutFullName.setOnClickListener(this@ProfileGuruFragment)
            layoutPassword.setOnClickListener(this@ProfileGuruFragment)
            layoutKelas.setOnClickListener(this@ProfileGuruFragment)
            layoutMapel.setOnClickListener(this@ProfileGuruFragment)
        }
    }
        ...
        viewModel.detailGuru(idGuru).observe(viewLifecycleOwner, { response ->
            ...
            if (response != null) {
                if (response.status == 200) {
                    ...
                    resulDetailGuru = response.result
                    prepareSiswa(result)
                } else {
                    showMessage(
                        requireActivity(),
    ...
    private fun observeMapel() {
        viewModel.mapel().observe(viewLifecycleOwner, { response ->
            ...
            if (response != null) {
                if (response.status == 200) {
                    // passing to list mapel
                    listMapel = response.results
                } else {
                    showMessage(
                        requireActivity(),
                        getString(R.string.failed),
    ...
    private fun prepareSiswa(result: ResultDetailGuru) {
        with(binding) {
            // set images
            ...
            tvNamaGuru.text = result.nama
            tvUsernameGuru.text = result.username
            tvPasswordGuru.text = result.password
            tvKelasGuru.text = result.namaKelas
            tvMapelGuru.text = result.namaMapel
        }
    }
    ...
    override fun onClick(v: View?) {
        with(binding) {
            when (v?.id) {
                ...
                R.id.layoutFullName -> {
                    val itemProfile = tvNamaGuru.text.toString().trim()
                    val titleSheet = resources.getString(R.string.titleName)
                    tvUiUpdated = tvNamaGuru
                    showBottomSheet(itemProfile, titleSheet, "nama", true)
                }
                R.id.layoutPassword -> {
                    val itemProfile = tvPasswordGuru.text.toString()
                    tvUiUpdated = tvPasswordGuru
                    showBottomSheet(
    ...
    private fun showBottomSheet(
        ...
        param: String? = null, isEditProfile: Boolean, isEditPassword: Boolean = false,
        isEditKelasAndMapel: Boolean = false
    ) {
        bottomSheetDialog = BottomSheetDialog(requireContext(), R.style.BottomSheetDialogTheme)

        if (isEditProfile) {
            when {
                isEditPassword -> { // change password in profile
                ...
                isEditKelasAndMapel -> {
                    ...
                    ) // layout edit kelas dan mapel

                    val layout = bottomSheetView.findViewById<RelativeLayout>(R.id.layoutEditKelas)
            ...
            ) // layout ubah foto profile
        }

        if (isEditProfile) {
            // ubah keterangan title bottom sheet selain password
            ...
            save(edtInput, itemProfile, param, isEditKelasAndMapel, isEditPassword)
    ...
    private fun save(edtInput: TextInputLayout?, itemProfile: String?, param: String?,
        ...
        btnSave.setOnClickListener {
            ...
            if (isEditPassword) { // cek jika edit password arahkan logic kesini
                ...
                when {
                    oldPassword.isEmpty() -> {
                        edtOldPassword.error = OLD_PASSWORD_MUST_BE_FILLED
                        return@setOnClickListener
                    }
                    newPassword.isEmpty() -> {
                        edtNewPassword.error = NEW_PASSWORD_MUST_BE_FILLED
                        return@setOnClickListener
                    }
                    newPasswordAgain.isEmpty() -> {
                        edtNewPasswordAgain.error =
                            NEWAGAIN_PASSWORD_MUST_BE_FILLED
                        return@setOnClickListener
                    }
                    else -> {
                ...
                if (isEditKelasAndMapel) {
                    // check kondisi field pada edit profile dan kelas-mapel
                    when {
                        idKelasFromSpinner == 0 -> {
                            spinnerKelas.errorText = KELAS_MUST_BE_FILLED
                            return@setOnClickListener
                        }
                        idMapelFromSpinner == 0 -> {
                            spinnerMapel.errorText = MAPEL_MUST_BE_FILLED
                            return@setOnClickListener
                        }
                        else -> getInput = idMapelFromSpinner.toString()
                    }
                } else {
            ...
            if (getInput.trim().isNotEmpty()) { // cek jika input tidak kosong
                // set ke partmap
                parameters[param!!] = getInput

                // save dan observe (hide btn save)
                loadingInBottomSheet(true)
                editProfile(getInput, parameters, isEditKelasAndMapel)
            }
        }
    }
    ...
    private fun setSpinnerKelas(itemKelas: List<ResultsKelas>?) {
        if (itemKelas != null) {
            ...
            val idSelectedKelas = listKelas!!.indexOf(
                ResultsKelas(
                    idKelas = resulDetailGuru.idKelas,
                    namaKelas = resulDetailGuru.namaKelas
                )
            )
    ...
    private val itemSelectedKelas = object : AdapterView.OnItemSelectedListener {
        override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
            ...
            if (listMapel != null) {
                ...
                if (idKelasFromSpinner != 0) {
                    spinnerKelas.errorText = null
                }
    ...
    private fun setSpinnerMapel(itemMapel: List<ResultsMapel>) {
        spinnerMapel = bottomSheetView.findViewById(R.id.spinnerMapel)

        if (itemMapel.isNotEmpty()) {
            spinnerMapel.item = itemMapel

            // pilihan item di awal
            val idSelectedMapel = filteredMapel.indexOf(
                ResultsMapel(
                    idKelas = resulDetailGuru.idKelas,
                    idMapel = resulDetailGuru.idMapel,
                    namaMapel = resulDetailGuru.namaMapel
                )
            )

            if (!selectedFromAPI) {
                spinnerMapel.setSelection(idSelectedMapel)
                selectedFromAPI = true
            }

            spinnerMapel.onItemSelectedListener = itemSelectedMapel
        } else {
            spinnerMapel.clearSelection()
            idMapelFromSpinner = 0 // set default ke 0
        }
    }
    ...
    private val itemSelectedMapel = object : AdapterView.OnItemSelectedListener {
        override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
            ...
            if (idMapelFromSpinner != 0) {
                spinnerMapel.errorText = null
            }
        }
    ...
    private fun editProfile(
        ...
        viewModel.changeDetailProfile(idGuru, newData)
                ...
                if (response != null) {
                    if (response.status == 200) {
                        if (isEditKelasAndMapel) {
                            ...
                            tvUiUpdated.text = newInputData.toString()
                        }
    ...
    private fun changeFotoProfile(result: Uri? = null) {
        with(binding) {
            ...
            if (result != null) {
                ...
                viewModel.changeFotoProfile(idGuru, body).observe(viewLifecycleOwner, { response ->
...
//                        observeDetailSiswa()
                ...
                viewModel.changeFotoProfile(idGuru, body).observe(viewLifecycleOwner, { response ->
                    ...
                    if (response != null) {
                        if (response.status == 200) {
                            binding.imgGuru.setImageResource(R.drawable.no_profile_images)
                            bottomSheetDialog.dismiss()

                            resulDetailGuru.fotoProfile = null
                        } else {
                            showMessage(
                                requireActivity(),
                                "Gagal",
    ...
    private fun isHasImage(): Boolean {
        return !resulDetailGuru.fotoProfile.isNullOrEmpty() || !currentImagePath.isNullOrEmpty()
    }
